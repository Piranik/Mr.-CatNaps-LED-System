...
const char *softAP_ssid = "MrCatNaps LED Setup";
const char *softAP_password = "12345678";
const char *softAP_ssid2 = "MrCatNaps LED Setup ";
...
#define DATA_PIN 15
int NUM_LEDS = 15;
#define NUM_LEDS_SET 15
#define CALIBRATION_TEMPERATURE TypicalLEDStrip
CRGB leds[NUM_LEDS_SET];
CRGB leds_new[NUM_LEDS_SET];
...
char aws_endpoint[]    = IOT ENDPOINT;
...
void messageArrived(MQTT::MessageData& md)
{
  ...
  String scene_text = state_text.substring(0, state_text.indexOf("\""));
...
void subscribe () {
   //subscript to a topic
    String aws_topic_start = "$aws/things/";
    String aws_topic_end  = "/shadow/update/delta";
    String aws_thing_name  = "mrcatnapsGMAILCOM";
    String bloop = aws_topic_start + aws_thing_name + aws_topic_end;
    std::string wtf = "$aws/things/mrcatnapsGMAILCOM/shadow/update/delta";
    char myConcatenation[48];
    char myCharArray[]="$aws/things/";
    char myCharArray3[]="/shadow/update/delta";
    sprintf(myConcatenation,"%s%s",myCharArray,thingName);
    sprintf(myConcatenation,"%s%s",myConcatenation,myCharArray3);
    Serial.println(myConcatenation);
    std::string str = myConcatenation;
...
void setup() {
  ...
  if(strlen(ledLength)>0){
    ledLengthInt = atoi( String(ledLength).c_str() );
    Serial.println("LED LENGTH int: ");
    Serial.println(ledLengthInt);
    NUM_LEDS = ledLengthInt;
    leds[ledLengthInt];
    leds_new[ledLengthInt];
...
//    WiFi.disconnect();
//    WiFiMulti.addAP("wtf is going on", "lkjdsaflk");
    Serial.println ("connecting to wifi");
    int internetTryTimes = 0;
    while (WiFiMulti.run() != WL_CONNECTED && internetTryTimes < 50 ) {
      delay(100);
      Serial.print (".");
      internetTryTimes += 1;
    }
    if (internetTryTimes > 49){
       internetConnected = false;
       Serial.println("Internet Connection Failed, Starting local...");
    }else{
       Serial.println ("\nconnected");
           //fill AWS parameters
  ...
  if(internetConnected == false){
      ...
      if(strlen(thingName)>0){
        WiFi.softAP(thingName, softAP_password);
      }else{
      ...
      //Mr. CatNaps server additions Start
      server.on("/led_control", ledControl);
      server.on("/color_check", colorCheck);
      server.on("/rainbow", rainbow_call);
      server.on("/glitter", rainbowWithGlitter_call);
      server.on("/sinelon", sinelon_call);
      server.on("/beats", bpm_call);
      server.on("/juggle", juggle_call);
      server.on("/beatwave", beatwave_call);
      server.on("/chaser", chaser_call);
      server.on("/fade", fade_call);
      server.on("/blend", blend_call);
      //Mr. CatNaps server additions End
      server.onNotFound ( handleNotFound );
      server.begin(); // Web server start
      Serial.println("HTTP server started");
      connecter = strlen(ssid) > 0; // Request WLAN connect if there is a SSID
  }
}
bool amzConnect = false;
void connectWifi() {
  delay(1000);
  internetConnected = false;
  loadCredentials(); // Load WLAN credentials from network
  Serial.println("LED LENGTH: ");
  Serial.println(String(ledLength));
  if(strlen(ssid)>0){
    //LED SETUP STUFF START
  if(strlen(ledLength)>0){
    ledLengthInt = atoi( String(ledLength).c_str() );
...
//    WiFiMulti.addAP("wtf is going on", "lkjdsaflk");
    ...
    if (internetTryTimes > 49){
       internetConnected = false;
       Serial.println("WE FAILED MISERABLY AND THAT IS A GOOD THING!");
    }else{
       Serial.println ("\nconnected");
           //fill AWS parameters
      awsWSclient.setAWSRegion(aws_region);
...
void loop() {
   if(internetConnected){
    ...
    {
      int s = WiFi.status();
      if (s == 0 && millis() > (lastConnectTry + 120000) ) {
...
void rainbow_call()
{
  ...
  if(internetConnected == false){
      server.sendHeader("Location", String("/led_control"), true);
